...
const MouldSummary = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${BASE}/MouldSummary/pm-summary-current-year`);
      setData(res.data);
    } catch (err) {
  ...
  const downloadExcel = () => {
    ...
    const workbook = XLSX.utils.table_to_book(table, { sheet: "Mould PM Summary" });
    XLSX.writeFile(workbook, `Mould_PM_Summary_${prodDate}.xlsx`);
  };
  ...
  const downloadPDF = () => {
    ...
    doc.save(`Mould_PM_Summary_${prodDate}.pdf`);
  };
        ...
        <div className="bg-white shadow-md rounded-xl p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            {/* <h2 className="text-xl font-semibold text-gray-700">
              Mould Preventive Maintenance Summary
            </h2> */}
